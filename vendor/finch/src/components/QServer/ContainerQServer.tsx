...
export default function ContainerQServer({ className }: ContainerQServerProps) {
    ...
    useEffect(() => {
        if (
            apiStatus &&
            (apiStatus.worker_environment_exists === false ||
                apiStatus.worker_environment_state === 'closed')
        ) {
            console.log(
    ...
    return (
        <main
            className={cn(
                'max-w-screen-3xl w-full min-w-[72rem] h-full min-h-[50rem] m-auto flex rounded-md relative bg-slate-400 border border-slate-400',
                className,
            )}
        >
            {/* ITEM POPUP  */}
