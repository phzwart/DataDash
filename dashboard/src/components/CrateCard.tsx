...
export default function CrateCard({
  ...
  return (
    <div className={`relative min-w-0 ${className}`}>
      <Link to={to} state={state} className="block no-underline h-full">
        <Paper
          className={`h-full border border-sky-400/70 ring-1 ring-sky-400/40 hover:border-sky-300 hover:ring-sky-300/60 transition-shadow bg-slate-900/60 ${
            compact ? "p-3" : "p-5"
          }`}
        >
          <div
            className={`flex items-start justify-between gap-2 ${overlay ? "pr-8" : ""}`}
          >
            <h2
          ...
          {gallery.show_id ? (
            <p
              className={`text-slate-400 font-mono ${
                compact ? "text-[10px] mt-1.5" : "text-xs mt-2"
              }`}
            >
              {shortId(crate.id)}
            </p>
            ...
            (gallery.fact_labels ? (
              <dl
                className={`text-slate-300 ${
                  compact ? "mt-2.5 space-y-1 text-xs" : "mt-4 space-y-1.5 text-sm"
                }`}
              >
              ...
              >
                {gallery.facts
                  ...
                  .reduce<ReactNode[]>((acc, node, i) => {
                    if (i > 0) {
                      // Hard spaces — not CSS margin (avoids Tailwind purge / collapse).
                      acc.push(
                        <span key={`gap-${i}`}>{"\u00A0\u00A0\u00A0\u00A0\u00A0\u00A0"}</span>,
                      );
                    }
