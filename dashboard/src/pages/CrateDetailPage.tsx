...
function MetaRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between gap-4 py-1.5 border-b border-slate-700/50 last:border-0">
      <dt className="text-slate-300 text-sm shrink-0 max-w-[45%]">{label}</dt>
      <dd className="text-slate-50 text-sm text-right break-all">{value}</dd>
    </div>
  );
}
...
function Section({
  ...
  return (
    <Paper className={`p-4 bg-slate-900/70 ${className}`}>
      <h2 className="text-sm font-semibold uppercase tracking-wide text-sky-200 mb-3">
        {title}
      </h2>
      {children}
    </Paper>
...
function KpiChip({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg bg-slate-800/80 border border-slate-700 px-3 py-2 min-w-[7rem]">
      ...
      <div className="text-sm text-slate-100 mt-0.5 font-medium truncate max-w-[14rem]">
        {value}
      </div>
    </div>
  );
...
function ClassFractionsSection({
  ...
  if (rows.length === 0) {
    return <p className="text-slate-500 text-sm">No class-fraction data</p>;
  }
      ...
      {rows.map((row) => (
        <div key={row.key}>
          <div className="flex justify-between gap-2 text-sm mb-1">
            ...
            <div className="text-right shrink-0 tabular-nums text-slate-300 text-sm">
              ...
              <div key={slotName}>
                {isUri && typeof raw === "string" ? (
                  <div className="flex justify-between gap-4 py-1.5 border-b border-slate-700/50">
                    <dt className="text-slate-300 text-sm shrink-0">
                    ...
                    <dd className="text-right">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-start gap-1 text-sky-300 hover:text-sky-200 text-sm break-all"
                      >
                        <LinkSimple size={14} className="shrink-0 mt-0.5" />
                        {display}
                      </a>
                    </dd>
                  </div>
  ...
  return (
    <div className="flex flex-col gap-4 p-4 w-full h-full min-h-0 overflow-auto">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {(dash?.hero.badges ?? []).map((slot) => (
            <span
              key={slot}
              className="text-xs uppercase tracking-wide px-3 py-1 rounded-full bg-sky-900/70 text-sky-100"
      ...
      {(dashQuery.isLoading || schemaQuery.isLoading) && (
        <p className="text-slate-400 text-sm">Loading dashboard / LinkML…</p>
      )}
      {dashQuery.error && (
      ...
      <div className="flex flex-col xl:flex-row gap-4 items-start">
              ...
              {hasCratePlots && crateCfg?.plots ? (
                <>
                  {sidecarQuery.isLoading ? (
                    <p className="text-slate-500 text-sm">Loading sidecar…</p>
                  ) : sidecarQuery.error ? (
        ...
        <div className="flex-1 min-w-0 grid gap-4 md:grid-cols-1 2xl:grid-cols-2 w-full">
          {schema && dash ? (
            ...
            <p className="text-slate-500 text-sm">Loading metadata sections…</p>
          )}
        </div>
      </div>
    </div>
