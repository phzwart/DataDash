...
export async function fetchCrateSidecar(
  ...
  // In Vite dev, use same-origin `/crate-assets` so the proxy handles it
  // (mirrors schema fetching — avoids CORS / wrong-origin surprises).
  const base = import.meta.env.DEV
    ? ""
    : getTiledOrigin();
  const url = new URL(
    `${base}/crate-assets/${encodeURIComponent(uuid)}/sidecar`,
